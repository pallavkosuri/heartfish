...
def fit_seed_points_base(im,centers,width_zxy=[1.,1.,1.],radius_fit=5,n_max_iter = 10,max_dist_th=0.25):
    ...
    if len(x)>0:
        ...
        return np.array([])
def get_ims_meds(folder,num_cols=None,fovs=None,median_fl=None,overwrite=False,tag='mid',verbose=False):
    ...
    if compute:
        ...
        imst_cols=[]
        if fovs is None:
            fovs = map(os.path.basename,glob.glob(folder+os.sep+'*.dax'))
        for fov in fovs:
            if verbose:
                print fov
            ims_ = io.dax_im(folder+os.sep+fov,num_col=num_cols).get_mids(tag=tag)
            imst_cols.append(ims_)
        im_meds=np.array(np.median(imst_cols,axis=0),dtype=np.uint16)
...
def linrescale(im_mean1,im_mean2,n=100000):
    ...
    return (im_mean2-c)/m
def return_candidate_centers(im,dist_th=20,ex=20,ey=20,ez=2,gmin=2,gmax=5,filtsz=5,ths=4,plt_val=False,save_file=None):
    zs,xs,ys = im.shape
    centersh = get_seed_points_base(im,gfilt_size_min=gmin, gfilt_size_max=gmax, filt_size=filtsz, th_std=ths,return_h=True)
    z,x,y,h = sparse_centers(centersh, dist_th=dist_th, brightness_th=0, max_num=np.inf)
...
def update_candid_spot(im,chr_pts_,txyz_,tag,th_std=2,num_pts=5,
                        cutoff_window=20,cutoff_chr=10,candid_spot={},drift_dic={},fl_cands=None,im_repair=None,plt_val=False):
    """im -> 3dimage of the current hybe
    chr_pts_ -> the postions of the selected chromosomes
    txyz_ -> drift from current hybe to reference frame where chromosome was selected 
             and to which the drift was computed to
    ...
    for ichr,chr_pt in enumerate(chr_pts_):
        ...
        if np.prod(im_sm.shape)>0:
            ...
            if im_repair is not None:
                im_sm = im_sm/im_repair[slices[1:]]
            im_medz = np.array(map(np.median,im_sm))[:,np.newaxis,np.newaxis]
            im_sm = im_sm/im_medz
...
def get_best_drift(drifts_):
    return [get_drift_median(drifts_,i) for i in range(np.max(map(len,drifts_)))]
...
def correct_color(dic_cand,htags,dic_chr={},dic_bleed={},
    ...
    cols = np.unique([tag.split('_')[-1] for tag in htags])
    chr_ms = {col:dic_chr.get('m_647_'+col,None) for col in cols}
    dic_cand_ = {}
    allhtags = np.unique(list(htags)+list(dic_cand.keys()))
    for htag in allhtags:
        col = htag.split('_')[-1]
